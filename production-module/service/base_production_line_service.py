...
class BaseProductionLineService:
    def __init__(self, name):
        ...
        self.producer = KafkaProducer(broker=os.environ.get("KAFKA_BROKER_ADDRESS"), topic=os.environ.get("KAFKA_TOPIC"))

    def start_production_line(self, config: Configurations):
        if self.countdown_thread is None or not self.countdown_thread.is_alive():
            self.producer.send_log("%s has been started. The amount remained: %i" % (self.operationName, config.amount))
            start_number = int(config.amount)
